...
uint8_t voice_present; // 0: iVoice absent, 1: iVoice Absent
...
int LoadGame(int entry_num) {
   ...
   voice_present = cart.RAM[VOICE_PRES_ADDR];
   audio_volume = cart.RAM[EMU_VOL_ADDR];
...
void RunLauncher() {
   ...
   cart.RAM[VOICE_EMU_ADDR] = CONFIG_INTELLIVOICE; // Intellivoice EMU available
